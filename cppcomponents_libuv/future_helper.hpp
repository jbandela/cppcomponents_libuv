...
//
#pragma once
#ifndef ASYNC_HELPER_FUTURE_HELPER_IMP_INCLUDED_04_24_2013
#define ASYNC_HELPER_FUTURE_HELPER_IMP_INCLUDED_04_24_2013
...
#undef PPL_HELPER_OUTPUT_ENTER_EXIT
	...
	class async_helper{
		...
		typedef std::function<future<T>()> func_type;
		co_ptr co_;
	public:
		...
		template<class R>
		R await(future<R> t,bool main_loop = true){
			PPL_HELPER_ENTER_EXIT;
			...
			return static_cast<detail::ret_type*>(co_->coroutine_caller_->get())->get<future<R>>().get();
		}
	...
	namespace detail{
		...
		class simple_async_function_holder : public coroutine_holder{
			...
			typedef typename future<return_type> future_t;
			...
			static void coroutine_function(coroutine_holder::co_type::caller_type& ca){
				...
				try{
					...
					func_type retfunc([ret](){
						return future_t([ret](){return ret.get(); });
					});
					ca(&retfunc);
				}
				catch (std::exception&){
	...
	template<class F>
	typename future<typename std::result_of<F(detail::convertible_to_async_helper)>::type> do_async(F f){
